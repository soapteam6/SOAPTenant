...
{
    /// <summary>
    /// Calculates and stamps soap_cost on a soap_salesorderdetail line.
    ///
    /// Registration (synchronous, Pre-operation, stage 20):
    ///   - Create  of soap_salesorderdetail
    ///   - Update  of soap_salesorderdetail  (filtered on soap_productid, soap_amount, soap_quantity)
    ///
    /// Pre-image name "PreImage" (Update step only):
    ///   soap_salesorderid, soap_productid, soap_amount, soap_quantity
    ///
    /// Logic:
    ///   1. Resolve soap_salesorderid → retrieve parent soap_salesorder.soap_pricelistid.
    ...
    /// </summary>
    public class OrderProductCostPlugin : PluginBase
    {
        private const int PricingMethodMarkup = 406670000;
        private const int PricingMethodMargin = 406670001;

        public OrderProductCostPlugin() : base(typeof(OrderProductCostPlugin)) { }
        ...
        protected override void ExecuteCdsPlugin(ILocalPluginContext localPluginContext)
        {
            ...
            {
                localPluginContext.Trace("OrderProductCostPlugin: Target is missing or invalid, skipping.");
                return;
            }
            ...
            {
                localPluginContext.Trace("OrderProductCostPlugin: No relevant field changed, skipping.");
                return;
            }
            ...
            localPluginContext.Trace($"OrderProductCostPlugin: Processing {message} on soap_salesorderdetail.");

            try
            {
                // ── 1. Resolve all field values (target wins over preImage) ──────────
                EntityReference salesorderRef = Resolve<EntityReference>(target, preImage, "soap_salesorderid");
                ...
                {
                    localPluginContext.Trace("OrderProductCostPlugin: soap_salesorderid not resolved, skipping.");
                    return;
                }

                decimal amount = amountMoney?.Value ?? 0m;

                // ── 2. Get soap_pricelistid from the parent soap_salesorder ────────────────
                EntityReference priceListRef = GetPriceListRef(service, salesorderRef.Id, localPluginContext);
                if (priceListRef == null)
                {
                    localPluginContext.Trace("OrderProductCostPlugin: soap_pricelistid not found on salesorder, skipping.");
                    return;
                }
                ...
                if (priceListItem != null)
                {
                    ...
                    localPluginContext.Trace(
                        $"OrderProductCostPlugin: List item found → cost = {itemAmountMoney.Value} × {quantity} = {cost}");
                }
                else
                {
                ...
                localPluginContext.Trace($"OrderProductCostPlugin: soap_cost set to {cost}.");

            }
            catch (Exception ex)
            {
                localPluginContext.Trace($"OrderProductCostPlugin Error: {ex.Message}");
                throw new InvalidPluginExecutionException($"Error in OrderProductCostPlugin: {ex.Message}", ex);
            }
        }

        // ── Private helpers ──────────────────────────────────────────────────────────

        /// <summary>Retrieves soap_pricelistid from the parent soap_salesorder.</summary>
        private EntityReference GetPriceListRef(
            IOrganizationService service, Guid salesorderId, ILocalPluginContext ctx)
        {
            Entity salesorder = service.Retrieve("soap_salesorder", salesorderId, new ColumnSet("soap_pricelistid"));
            EntityReference priceListRef = salesorder.GetAttributeValue<EntityReference>("soap_pricelistid");
            ctx.Trace($"OrderProductCostPlugin: soap_pricelistid = {priceListRef?.Id.ToString() ?? "null"}");
            return priceListRef;
        }
            ...
            IOrganizationService service, Guid priceListId, Guid productId, ILocalPluginContext ctx)
        {
            ...
            ctx.Trace($"OrderProductCostPlugin: soap_pricelistitem lookup → {result.Entities.Count} row(s).");
            ...
            IOrganizationService service, Guid priceListId, decimal amount, ILocalPluginContext ctx)
        {
            ...
            ctx.Trace($"OrderProductCostPlugin: Pricing method = {method}, percent = {percent}");

            if (method == PricingMethodMarkup)
            {
                decimal divisor = 1m + (percent / 100m);
                decimal cost = divisor == 0m ? 0m : amount / divisor;
                ctx.Trace($"OrderProductCostPlugin: Markup → {amount} / (1 + {percent}/100) = {cost}");
                return cost;
            }
            ...
            if (method == PricingMethodMargin)
            {
                ...
                ctx.Trace($"OrderProductCostPlugin: Margin → {amount} × (1 - {percent}/100) = {cost}");
                return cost;
            }
